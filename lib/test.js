...
describe('WAPI MAIL nodeJS SDK', function() {
    ...
    it('environment variable should be setten up', () => {
        assert.strictEqual(process.env.WAPI_MAIL_SENDER_ID, "Lorl6h8");
    });
    ...
    describe('payload', () => {
        ...
        it('exp attribut should be less to a timestamp in secounds in a minute from now', () => {
            assert.strictEqual(true, valid_payload.exp < (Math.floor(Date.now() / 1000) + 60));
        });
    ...
    describe('jwt', () => {
        ...
        it('the token produce by the makeToken Fn should be a valid and issue a valid payload', () => {
